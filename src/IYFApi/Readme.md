...
WORKDIR /src
COPY ["IYFApi.csproj", "IYFApi/"]
RUN dotnet restore "IYFApi/IYFApi.csproj"

WORKDIR "/src/IYFApi"
COPY . .
RUN dotnet build "IYFApi.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "IYFApi.csproj" \
...
Execute unit tests
```
    cd "IYFApi/test/IYFApi.Tests"
    dotnet test
```

Deploy application
```
    cd "IYFApi/src/IYFApi"
